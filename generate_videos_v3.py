# generate_videos_v3.py
# Targets inputs for the ORIGINAL WanVideo workflow using config_v3.py.
...
# --- Import project config (V3) and helpers ---
try:
    import config_v3 as config # Use the V3 config
...
# --- Constants ---
LOG_FILE = getattr(config, 'LOG_FILE', Path("logs") / "video_gen_v3.log") # Use a different log name maybe
...
VIDEO_OUTPUT_SAVE_NODE_TITLE = config.VIDEO_OUTPUT_SAVE_NODE_TITLE # e.g., "mp4"
...
def run_video_generation(project_path: Path):
    log_step(f"--- Step 1: Setup & Validation (v3) ---", l="info", i=True)
    ...
    log_step(f"--- Step 2: Generating Videos (v3) ---", l="info", i=True); success_count = 0; fail_count = 0; processed_count = 0; at_least_one_comfy_run_started = False
    for image_path in approved_image_paths:
        ...
        try:
            ...
            # --- Prepare node modifications for the ORIGINAL WanVideo Workflow ---
            inputs_to_set = {
                # MODIFIED: Target the SINGLE prompt node and set its SPECIFIC input fields
                config.VIDEO_PROMPT_NODE_TITLE: {
                    "positive_prompt": positive_prompt,  # Set the 'positive_prompt' field
                    "negative_prompt": negative_prompt   # Set the 'negative_prompt' field
                },

                # Target the Sampler node by title for the seed
                config.VIDEO_SEED_NODE_TITLE: {"seed": seed}, # Targets Node #27 'seed'

                # Target the LoadImage node by title for the image filename
                config.VIDEO_START_IMAGE_NODE_TITLE: {"image": start_image_path.name}, # Targets Node #341 'image'

                # MODIFIED: Target the Video Combine node by its title (e.g., "mp4") for output naming
                ...
                } # Targets Node #30 'filename_prefix'
            }
            log_step(f" Modifying workflow inputs (v3 method)...", l="info")
    ...
    log_step("\n--- Finished Video Generation Phase (v3) ---", l="info", i=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Videos using ComfyUI (WanVideo - v3)."); parser.add_argument("-p", "--project_path", help="Project folder. Omit for latest.")
    args = parser.parse_args(); log_step("===== Generate Videos Script Started (v3) =====", l="info", i=True)
    ...
    # Update required titles validation for v3 config
    required_titles = [config.VIDEO_PROMPT_NODE_TITLE,
                       config.VIDEO_SEED_NODE_TITLE,
                       config.VIDEO_START_IMAGE_NODE_TITLE,
                       config.VIDEO_OUTPUT_SAVE_NODE_TITLE]

    if not all(required_titles): log_step(f"CRITICAL Error: Required VIDEO node titles missing in config_v3: Need {required_titles}", l="error", i=True); exit(1)
    ...
    if success: log_step("Script finished successfully (v3).", l="success", i=True); sys.exit(0)
    else: log_step("Script finished with errors (v3).", l="error", i=True); sys.exit(1)